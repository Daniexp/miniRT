...
unsigned int	*difuseColor(t_light *L, float *p, float *N, float kd, unsigned int *rgb)
{
	float	*light_vect;
	int		i;
	unsigned int		*clr;
	...
	i = -1;
	while (++i < 3)
		clr[i] = (unsigned int) roundl(kd * L->rate * fmax(0.0, scalar_product(light_vect, N)) * rgb[i]);
	return (clr);
}
